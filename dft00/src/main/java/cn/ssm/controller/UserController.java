...
public class UserController {  
    ...
    public String toIndex(HttpServletRequest request,Model model){  
        int userId = Integer.parseInt(request.getParameter("id"));  
        System.out.println("userId:"+userId);
        User_t user = this.userService.getUser_tById(userId);  
        log.debug(user.toString());
        model.addAttribute("user", user);  
        return "showUser";  
    }  
 ...
 // /user/{id}
    ...
    public @ResponseBody User_t getUserInJson(@PathVariable String id,Map<String, Object> model){  
        int userId = Integer.parseInt(id);  
        System.out.println("userId:"+userId);
        User_t user = this.userService.getUser_tById(userId);  
    ...
    public ResponseEntity<User_t>  getUserInJson2(@PathVariable String id,Map<String, Object> model){  
        int userId = Integer.parseInt(id);  
        System.out.println("userId:"+userId);
        User_t user = this.userService.getUser_tById(userId);  
        log.info(user.toString());
        return new ResponseEntity<User_t>(user,HttpStatus.OK);  
    } 
